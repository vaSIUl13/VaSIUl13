...
const MenuPage = ({ onAddToCart }) => {
  ...
  return (
    <div id="menu" className="menu-page">
      <h1 className="title">Наше Меню</h1>
      ...
      <div className="filter-buttons">
        ...
        <button onClick={() => setCategory("паста")}>Паста</button>
        <button onClick={() => setCategory("боули")}>Боули</button>
        <button onClick={() => setCategory("бургери")}>Бургери</button>
      </div>

      <div className="dishes-grid">
        {filteredDishes.map((item) => (
          <DishCard key={item.id} dish={item} onAddToCart={onAddToCart} />
        ))}
      </div>
    </div>
  );
